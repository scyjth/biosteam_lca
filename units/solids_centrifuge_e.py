...
from biosteam.units import SolidsCentrifuge
import numpy as np
...
base_design = SolidsCentrifuge._design
...
def baseLCA(self, location=None):
    ...
    * 'Process': (g CO2eq)
    ...
    W = self._Design['Weight of centrifuge (lb)']
    GHG['Construction emission'] = {}
    if W:
        GHG['Construction emission']['SolidsCentrifuge'] = (W *flow.steel_e()['emission(g CO2eq)'] *0.0005)
    self._GHGs =GHG
...
# %% Replace functioins
SolidsCentrifuge.rho_M = construction.rho_vessel
SolidsCentrifuge._design = _design
SolidsCentrifuge.energy_inputs=energy_inputs
SolidsCentrifuge.baseLCA =baseLCA
SolidsCentrifuge._GHG_units = {'Process emission': 'g CO2eq/hr',
                           'Construction emission': 'g CO2eq'} 
SolidsCentrifuge.multiLCA = multiLCA
